ccimport { useState, useEffect } from 'react'
  ...
  if (selectedForce && selectedNeighbourhood) {
    ...
    fetch(`https://data.police.uk/api/${selectedForce}/${selectedNeighbourhood}`)
      ...
      .then(data => {
        ...
        // Finally fetch boundary and crime stats
        return fetch(`https://data.police.uk/api/${selectedForce}/${selectedNeighbourhood}/boundary`)
          ...
          .then(boundaryData => {
            if (boundaryData.length > 0) {
              const { latitude, longitude } = boundaryData[0];
              const date = new Date();
              date.setMonth(date.getMonth() - 1);
              const monthString = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
              
              return fetch(
                `https://data.police.uk/api/crimes-street/all-crime?lat=${latitude}&lng=${longitude}&date=${monthString}`
              );
            }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8 text-center">
        <p className="text-gray-600 dark:text-gray-300 mb-4 leading-relaxed">
          Welcome to the UK Police Data Explorer, created by Alex Orr as part of the JHUB Module 1B - Creating a Rich Web Application.
        </p>
        <div className="prose dark:prose-invert max-w-none text-gray-700 dark:text-gray-300">
          <p className="mb-4">
            This interactive application provides real-time access to UK police force data through the official Police API. Features include:
          </p>
          <div className="grid md:grid-cols-2 gap-4 text-left max-w-2xl mx-auto mb-4">
            <ul className="list-disc pl-5 space-y-2">
              <li>Browse all UK police forces</li>
        ...
        {neighbourhoodData && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            ...
            <div className="col-span-full">
              ...
              {/* Crime Statistics */}
              <div className="mt-6">
                <h4 className="font-medium mb-4 dark:text-white">Crime Statistics (Last Month)</h4>
                <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                  {crimeStats.length > 0 ? (
                    <div>
